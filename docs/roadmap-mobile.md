...
| **S2 手機檢視** | ✅ 已完成（2026-08-02）。PWA 讀全部白板、列出卡片；唯讀。見下節 | 在外**查全部** | 2 天 |
| **S3 手機簡單編輯** | ✅ 已完成（2026-08-02）。勾待辦、改文字卡；帶卡片級合併。見下節 | **看全部 + 簡單編輯** 達成 | 2–3 天 |
...
## S2 手機讀全部白板（2026-08-02 完成）

手機端原本只是一個**單向投遞口**：東西進得去，但看不到任何既有內容。S2 補上讀取。

### 為什麼不必打包 tldraw

卡片是 snapshot store 裡的 `card` shape，而**讀它只需要純 JSON 操作**——
`getCardShapes` 是純函式，不碰 canvas。這是 S0(a) 把卡片擷取抽成純函式時就鋪好的路。
手機 bundle 419 kB → **430 kB**（gzip +3 kB），仍然不含 tldraw。

### 三個新東西

| 檔案 | 做什麼 |
|---|---|
| `mobileSyncCore.fetchBoardsCore()` | 白板清單。⚠️ `select` **不含 snapshot 也不含 thumbnail**——清單只需要名字，snapshot 是整列最大的東西 |
| `mobileSyncCore.fetchBoardSnapshotCore(id)` | 單塊板的 snapshot。唯一會下載大量資料的呼叫，只在真的點進去時才發生 |
| `mobileCards.ts` | snapshot → 顯示模型（純函式，21 條測試） |
| `MobileBrowse.tsx` | 白板清單／卡片列表兩個畫面 |

`authedGet` 沿用速記那套兩段式 token 處理（先確保沒過期，真撞到 401 再換一次重試），
否則會出現「速記送得出去、清單卻 401」這種只壞一半的狀態。

### 兩個踩到的坑（都被測試當場抓到）

1. **`stripHtml` 會把區塊元素之間壓成一個空格**。第一版寫成「剝完 HTML 再取第一行」
   當標題，結果 `<h2>標題</h2><p>內文</p>` 剝完是「標題 內文」一整行，永遠切不出標題。
   必須在剝之前先看 H1/H2（改用桌機既有的 `splitTitleBody`）。
2. **`stripHtml` 連純文字裡的換行也壓成空格**。所以純文字的 fallback 要
   **先按換行切、再逐行剝**。

另外 `mobileCards` 因為用到 `stripHtml`（內部是 DOMParser）**不能進 service worker**；
速記的送出路徑刻意不碰它，`sw.ts` 才能維持自足。

### 卡片順序照畫布位置排

snapshot store 是物件，鍵順序不保證與視覺順序一致。不排的話手機上的順序會跟桌機
看到的排版完全對不起來，同一塊板在兩邊像兩塊板。容差 40px：同一「列」的卡片
高度常有些微差異，不容差會被拆成好幾列。

### 刻意的限制

- **圖片與檔案卡只顯示檔名**，並明說「只能在桌機開啟」——實體檔在桌機的 userData，
  手機既沒有檔案系統也沒有 `electronAPI`。做成看起來可以點的樣子等於承諾做不到的事。
  （手機要看到圖需要 Storage 簽名 URL，是另一件事。）
- 資料夾不進清單：手機端不做層級，平鋪比較好按。
- 墓碑與垃圾桶裡的板（都有 `deleted_at`）一律排除。

**端到端實測**：把手機 PWA 用 `vite preview` 起在本機，導進 Electron 的 Chromium、
注入桌機現成的 session ⇒ 白板清單列出 **19 塊板**、點進「進度 · 2026-07-29」列出
**13 張卡（2 項未完成、28 行待辦）**，型別標籤、勾選狀態、刪除線都正確。

---

## S3 手機簡單編輯（2026-08-02 完成）

### 真正的工作量在合併，不在 UI

手機一旦能寫，手機與桌機就會產生**真的衝突**。桌機那套卡片級合併靠
`syncState.shapeHashes` 當共同祖先，但那份存在桌機的 localStorage，手機拿不到。
手機端要有等價的一份，否則每次寫入都退回「整塊覆蓋」，把桌機這段期間的修改蓋掉。

**手機的 base 從哪來**：`fetchBoardSnapshotCore` 在**讀那塊板的當下**就把指紋算好
一起回傳，存在記憶體裡帶著走。編輯 session 很短（讀 → 改 → 存），不需要跨重啟——
App 重開就重讀一次，那本來就會拿到新的 base。

**寫回流程**（`saveBoardWithMergeCore`）與桌機的 pushPhase 同構：

    拉雲端最新版 → 三方合併（base = 讀進來時的指紋）→ 推合併結果

同一張卡兩邊都改時**讓遠端贏**（`preferRemoteOnConflict: true`）——手機上多半是
隨手一改，桌機那邊通常是正在專心做的版本，衝突時保守一點。
`updatedAt` 推進到現在，否則桌機會覺得「我的比較新」再蓋一次。

### 沒有離線編輯佇列，這是刻意的

速記有 outbox 是因為「打完就關掉」很常見。編輯則是使用者**正看著那張卡**，
當場告訴他沒存成功、內容留在畫面上讓他重試，比默默排隊之後再合併誠實得多。
（離線編輯佇列要處理「排隊期間 base 過期」，是另一個等級的複雜度。）

### 純函式層：`utils/snapshotPatch.ts`

桌機改卡片走 `editor.updateShape`，那需要活著的 tldraw 編輯器；手機只能直接改
snapshot JSON。這一層就是那條路，**一律回新物件不 mutate**——理由不是潔癖：
合併需要拿「改之前」與「改之後」比對，就地改掉的話 base 會跟著變。

⚠️ **勾待辦一定要用 todo 的 id 定位，不是陣列索引**。索引會隨著另一端新增/刪除
項目而位移，拿索引去勾等於「勾到當時排在第 N 個的那一項」。沒有 id 的舊資料
在手機上**不給勾**（`mobileCards` 直接濾掉）——無法安全定位就不要提供操作。

⚠️ `props.text` 的約定一直是 HTML。手機編輯的是純文字，存回去前要包成 `<p>` 段落
並跳脫特殊字元；直接塞純文字進去的話，桌機用 TipTap 讀它會把整段當一行、換行全沒。

### 能編輯的範圍

只有**文字類**卡片（text / journal / sticky / heading）與**待辦勾選**。
表格、顏色樣本那些的結構沒辦法用一個 textarea 表達，硬給編輯入口只會讓人把資料改壞。

**端到端實測**：種一塊三張卡的測試板 → 手機進去 → **在手機編輯期間直接改雲端的
另一張卡**（模擬桌機）→ 手機勾一個待辦 → 雲端結果：手機勾的生效、**桌機那張的
修改完整保留**、沒動的那張原封不動。接著在手機改文字卡 → 存成 `<p>` 兩段，
前面兩項依然都在。測試板已從雲端刪除。
