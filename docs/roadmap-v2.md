...
1. **⚠️ 動手前發現一個沒被記錄過的洞：`buildSearchIndex` 只索引 5 種型別**
   （text／todo／link／image／journal），其餘 **6 種**（heading／sticky／table／color／board／file）
   被 `continue` 直接跳過＝**根本搜不到**。這個 bug 不會報錯、不會當機，只會讓人覺得
   「我明明記過這件事」然後懷疑自己記錯。**實測影響**：搜「同步」原本回 14 筆，
   修正後 19 筆，多出來的 5 筆全是便利貼——而便利貼正是使用最頻繁的型別之一。
   因此 B3 的真正前置是「先把索引補齊」，否則篩選列只列得出 5 種，反而更讓人困惑。
2. **chip 只列「這次關鍵字真的搜得到的型別」**（`typeCountsFor`），而非固定列出全部 11 種——
   列出永遠 0 筆的 chip 只會讓人以為篩選壞了。命中型別只有一種時整列不顯示。
3. **順手納入標籤**：`tags` 對所有型別都進索引，搜「閱讀」找得到標了 `#閱讀` 的卡。
4. **順手消除重複**：SearchPanel 原本自帶一份只有 5 種型別的 `typeIcon`，改用共用的
   `utils/cardMeta.ts`（該檔註解本來就寫著「避免同一份對照表在三個面板各抄一次」）。
...
| P-DRAW（D8） | 筆刷卡頓 | ✅ **已關閉（2026-07-17）** | 無 | **使用者在日常 electron-dev + 真實 vault 實測回報「正常、不卡了」**，依 2026-07-15 預訂的判準關閉，記為 **TD-IMG（commit `7eaf7f5`）附帶治好**——卡頓回報時圖片是 base64 內嵌（白屏 OOM 同一元兇），現走 `astro-img://` 串流不進 heap。原假說「重型卡片元件在畫圖時重繪」方向正確，病根與 OOM 同源。**未測成功的「200 卡＋400 筆」組合不再補測**（該組合的目的是逼出真實白板的卡頓，而真實白板已不卡）。**⚠️ [manual-test-2026-07-04.md](maintenance/manual-test-2026-07-04.md) D8 的「量測方法的三個坑」刻意保留——對未來任何效能量測都適用，重跑前必讀**。2026-07-15 CDP 量測（佐證，非依據）：①卡片數 0／60／200 張文字卡 ②空白板連畫 400 筆（+0.5ms）③單筆 900 點路徑（−0.1ms）——皆穩定 60fps、掉幀 0，即「文字卡上不重現」，與使用者回報一致 |
...
| N5 | Smart Collections 智慧集合 | 🟢 低（2026-07-15 下調） | 無 | **原定位有誤**：被寫成篩選器功能，但真正需要的是「主動浮現」（見 [product-redesign-2026-07.md](product-redesign-2026-07.md)）——浮現的部分已改由儀表板重組＋N6 承接（**⚠️ 2026-07-17 更新：N6 已結案，只交付到「`[[卡片名]]` 能連能跳」＝階段 1；「主動浮現漏連的舊卡」那部分沒有人承接了。若日後又想要「主動浮現」，這裡是空的，需重新決定由誰做**）。剩下的「逾期/高優先」預設集合對本使用者恆為空（無 dueDate）；「孤立卡片」短期是雜訊（2026-07 才開始用 `[[]]`，無 backlink ≈ 全部舊卡）。可做可不做 |
| N6 | 未連結提及偵測（Unlinked mentions） | ✅ **結案（2026-07-17，使用者拍板）** — 階段 1 已交付，**階段 2/3 不做** | — | **⚠️ 本項已關閉，勿再排程。** **2026-07-17 使用者拍板：N6 整項結案，不做階段 2（唯讀提示）與階段 3（批次改寫）。** 實際交付的是**階段 1**：修好 B-LINK 讓 `[[卡片名]]` 真的能跳、補全納入卡片名（commit `6e06ee1`）——**「能連」這件事本身就是使用者要的**，而階段 2/3 要解的「找出漏連的舊卡」需求，使用者判斷不必做。**技術前置全部已解除且無浪費**：效能已驗（見 [n6-performance-2026-07-15.md](maintenance/n6-performance-2026-07-15.md)），階段 1 順手把 `scanBoard` 的 stripHtml 從 1~2 次收斂成 1 次＝**useBacklinks 反而快 32%**，`cardIndex` 也留在 `useBacklinks` 供跳轉/補全使用。**若未來重啟**：階段 2 的難點是比對準確度（CJK 無詞邊界）、階段 3 的難點是純文字位置對不回 HTML 位置（須走 DOM 逐 text node，不可 `html.replace`）；下方保留原始論證脈絡。~~重新定位：這不是圖譜裝飾，是「憶起」的主力機制~~（見 [ADR 0007](adr/0007-cards-bound-to-single-board.md)）。寫下 `[[專案A]]` 時指出「另有 3 張舊卡提到過但沒連結」＝用新連結打撈舊卡，且自帶時機（寫作當下浮現，不需使用者記得去看）。比「久未造訪」高明：後者靠時間推東西，常推出已結案的死板＝雜訊；前者靠語意相關。~~前置：需驗證能否掛上 `useBacklinks` 增量快取~~ → **✅ 已實測，效能非問題**（見 [n6-performance-2026-07-15.md](maintenance/n6-performance-2026-07-15.md)）：`stripHtml` 是**沉沒成本**（`scanBoard` 今天就對每張卡跑 DOMParser，N6 要的純文字正是那個算完被丟掉的東西）；真實粒度（board-level 失效）下每次編輯 1000 卡 < 8ms、3000 卡 22.8ms；**掛上去反而讓 useBacklinks 快 32%**（現在有連結的卡 strip 兩次）。**風險改為「準確度／雜訊」**：樸素子字串比對會把「主頁白板」誤判進「主頁白板 (2)」的內文，且句子型標題全數落空 → 下一個要解的是**比對規則**，不是速度 |
...
| N9 | Diagnostics / Debug 面板 | 🟢 低（2026-07-17 下調） | 無 | main.js 已有 console 轉發/崩潰監聽；面板化＋debug report。**下調原因：原本的主要理由是「支援 D8 排錯」，而 D8 已於 2026-07-17 關閉**，目前沒有待排的效能/崩潰問題在等這個面板 |
