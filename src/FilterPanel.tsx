...
function parseCardStatus(value: unknown): CardStatusType {
    return typeof value === 'string' && CARD_STATUSES.includes(value as CardStatusType) ? (value as CardStatusType) : 'none'
}

function parsePriority(value: unknown): PriorityType {
    return typeof value === 'string' && PRIORITIES.includes(value as PriorityType) ? (value as PriorityType) : 'none'
}
    ...
    for (const board of boards) {
        for (const shape of getCardShapes(board.snapshot)) {
            ...
            if (matchStatus && matchPriority && matchTag) {
                let preview = ''
                if (ptype === 'text' || ptype === 'journal') preview = stripHtml(shape.props.text || '').slice(0, 60)
                else if (ptype === 'todo') preview = shape.props.text || shape.props.todos?.[0]?.text || ''
                results.push({ boardId: board.id, boardName: board.name, shapeId: shape.id, preview, cardStatus: status, priority, tags, x: shape.x ?? 0, y: shape.y ?? 0 })
            }
        }
    }
    return results
...
function FilterResultRow({ result, onJump, isDark }: FilterResultRowProps) {
    ...
    return (
        ...
        >
            ...
            <div style={{ fontSize: 12, color: textColor, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: result.tags.length > 0 ? 4 : 0 }}>
                {result.preview || '(無內容)'}
            </div>
...
export function FilterPanel({ boards, onJump, onClose, isDark }: FilterPanelProps) {
    ...
    const toggleStatus = (s: CardStatusType) =>
        setFilterStatuses(prev => { const n = new Set(prev); if (n.has(s)) n.delete(s); else n.add(s); return n })

    const togglePriority = (p: PriorityType) =>
        setFilterPriorities(prev => { const n = new Set(prev); if (n.has(p)) n.delete(p); else n.add(p); return n })

    const clearAll = () => { setFilterStatuses(new Set()); setFilterPriorities(new Set()); setFilterTag(null) }
        ...
        <div style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
                {!hasFilter ? (
                    ...
                    <div style={{ padding: '36px 16px', textAlign: 'center', color: '#ccc', fontSize: 13 }}>沒有符合條件的卡片</div>
                ) : (
                    <>
