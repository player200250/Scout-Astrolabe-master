...
| **S1 手機捕捉** | ✅ **已完成（2026-07-27）**：極簡 PWA，登入 → 速記丟進收件匣 → 同步回桌機。見 [mobile-pwa.md](mobile-pwa.md) | **出門速記**（命中最大痛點） | 1.5 天 |
...
**尚未做**：圖片接 Supabase Storage（`image` 卡目前只同步 `storedName`，手機看不到圖）、卡片級合併（現為整板 LWW）。
