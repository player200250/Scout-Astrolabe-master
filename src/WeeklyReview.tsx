...
export function getWeekRange(date: Date): { start: Date; end: Date; weekNum: number } {
    ...
    // ISO week number via nearest Thursday
...
function computeWeekStats(boards: BoardRecord[], weekStart: Date, weekEnd: Date): WeekStats {
    ...
    for (const board of boards) {
        ...
        for (const shape of Object.values(store) as any[]) {
            ...
            if (props.type === 'todo' && Array.isArray(props.todos)) {
                completedTodos += props.todos.filter((t: any) => t.checked).length
            }

            if (props.text) {
                const matches = (props.text as string).match(/\[\[[^\]]+\]\]/g)
                if (matches) wikiLinks += matches.length
            }
        }

        if (boardCards > 0) cardsByBoard.push({ boardName: board.name, count: boardCards })
    }
...
export function WeeklyReview({ boards, sidebarWidth, onClose, onGoToWeeklyCard }: WeeklyReviewProps) {
    ...
    const statCard = (bg: string, icon: string, label: string, value: string | number, valueColor: string) => (
        <div style={{
            ...
            <span style={{ marginLeft: 'auto', fontSize: 16, fontWeight: 700, color: valueColor }}>
                {value}
            </span>
        </div>
    )
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: stats.cardsByBoard.length > 0 ? 8 : 0 }}>
                            ...
                            <span style={{ marginLeft: 'auto', fontSize: 16, fontWeight: 700, color: '#2563eb' }}>
                                {stats.totalCards}
                            </span>
                        </div>
