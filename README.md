...
| 📥 收件匣 | 專屬白板作為未分類卡片的暫存區 |
| 🔒 自動備份 | 切換白板 / 關閉 App 時自動備份，最多保留 5 份，可還原 |
...
> 每份備份是「所有白板的完整快照」的複製，份數過多會把 IndexedDB 撐到數 GB 並在寫入時造成記憶體尖峰。
> 上限於 2026-06-21 由 30 降為 5（見 `docs/maintenance/bugs.md` P1-OOM）。
...
**Q: 如何備份資料？**
A: 點側邊欄「🔒」查看備份記錄，App 會自動保留最近 5 份備份。
...
**白板核心**
- [x] 多白板管理（釘選/封存/拖曳排序）
- [x] 收件匣白板（Ctrl+Shift+I）
- [x] 白板總覽（Ctrl+Shift+O）

**卡片系統**
- [x] 8 種卡片類型（文字/圖片/Todo/連結/Board/Journal/標題）
- [x] 卡片屬性（顏色/狀態/優先級/標籤）
- [x] 卡片模板（5個內建+自訂）
...
**系統功能**
- [x] 主頁儀表板（統計/日記/待辦/最近白板）
- [x] 垃圾桶（軟刪除14天保護）
- [x] Quick Capture（Ctrl+Space）
- [x] 自動備份/還原（最多30份）
- [x] 匯出（JSON/PNG/PDF/Markdown）
- [x] 暗色模式
- [x] 新手導覽
