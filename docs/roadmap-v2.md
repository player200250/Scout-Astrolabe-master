...
| P-DRAW（D8） | 筆刷卡頓 | ⏸ **結論未定，等使用者回報**（2026-07-15 已量測但方法有洞） | 無 | **文字卡上不重現**（可採信）：①卡片數 0／60／**200 張文字卡** ②筆畫累積（空白板連畫 **400 筆**，+0.5ms）③單筆 900 點路徑（−0.1ms）——皆穩定 60fps、掉幀 0。**最可能已被 TD-IMG 順便治好**（卡頓回報時圖片是 base64 內嵌，現走 `astro-img://` 串流不進 heap）。**⚠️ 但「200 卡＋400 筆」的組合沒測成功**（最貼近真實白板的情境），且**量測方法踩過三個坑**——最嚴重的是**視窗被遮蔽時 Chromium 停掉 rAF ⇒ 量出假的「60fps 掉幀 0」**（沒渲染當然不掉幀）。**看數字前務必先讀** [manual-test-2026-07-04.md](maintenance/manual-test-2026-07-04.md) D8 的「量測方法的三個坑」。**下一步成本最低的在使用者**：在日常 electron-dev + 真實 vault 上找當初會卡的板畫十秒 → 不卡就關掉此項；還卡才值得投入 |
